...
setup() {
    ...
    echo '  if [[ "$3" == "s3://test-bucket/test-cache-key/test-cache-key.tar.zst" ]]; then' >> mock_aws/aws
    echo '    echo "Found test-cache-key.tar.zst"' >> mock_aws/aws
    ...
    echo '  if [[ "$3" == "s3://test-bucket/test-cache-key/test-cache-key.tar.zst" ]]; then' >> mock_aws/aws
    echo '    touch test-cache-key.tar.zst' >> mock_aws/aws
    echo '    exit 0' >> mock_aws/aws
    echo '  else' >> mock_aws/aws
    echo '    exit 1' >> mock_aws/aws
    echo '  fi' >> mock_aws/aws
...
@test "Download the archive successfully if it exists" {
    ...
    run ./test-script.sh
    [ "$status" -eq 0 ]
    [[ "$output" == *"Cache archive successfully downloaded - test-cache-key.tar.zst"* ]]
    [ -f "test-cache-key.tar.zst" ]
}
